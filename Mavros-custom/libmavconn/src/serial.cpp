	...
	for (size_t i = 0; i < bytes_transferred; i++) {
		if (mavlink_parse_char(channel, rx_buf[i], &message, &status)) {
			logDebug(PFXd "recv: Message-Id: %d [%d bytes] Sys-Id: %d Comp-Id: %d Seq: %d",
             ...
             //ROS_INFO("######## serial Message chan:[%d]",channel);
             //ROS_INFO("ID: %d", message.msgid);

			/* emit */ message_received(&message, message.sysid, message.compid);
		}
/*		
		else {
			if((rx_buf[i] == 254) && (rx_buf[i+3] == 1) && (rx_buf[i+4] == 1) && (rx_buf[i+5] == 220) && (i < bytes_transferred - 5)){
				ROS_INFO("ID: %d", rx_buf[i+5]);
				ROS_INFO("message: [%d,%d,%d,%d,%d]", rx_buf[i+1],rx_buf[i+2],rx_buf[i+3],rx_buf[i+4],rx_buf[i+5]);

				mavlink_status_t* status = mavlink_get_channel_status(channel);
				ROS_INFO("STATE: %d", status->parse_state);
			}
